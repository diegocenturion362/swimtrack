...
export function MySeries() {
  const { id: swimmerId = '' } = useParams<{ id: string }>()
  ...
  const groups: SimilarSetComparison[] = useMemo(() => {
    const swSessions = sessions.filter(s => s.swimmerId === swimmerId && !s.esPlaneada)
  ...
  return (
    <>
      <Header title="Mis series" subtitle="Comparar evolución" showBack backTo={`/nadador/${swimmerId}`} />
      <PageLayout>

        <p className="text-xs text-slate-500 mb-4">
          Buscá una serie para ver cómo evolucionaron tus tiempos. Escribís <strong>8x50</strong>, <strong>50</strong> o <strong>100 mariposa</strong> y agrupa todas las variantes similares.
        </p>
        ...
        <div className="relative mb-3">
          ...
          <input
            ...
            placeholder="Buscar: 8x50, 100 mariposa, libre…"
          ...
          {query && (
            <button
              onClick={() => { setQuery(''); setSelectedKey('') }}
        ...
        {isSearching && (
          <p className="text-xs text-blue-600 mb-3 font-medium">
            🔍 Series similares a "{query}" — agrupadas por distancia y estilo, sin importar las repeticiones.
          </p>
        )}
        ...
        {groups.length === 0 ? (
          <Card className="text-center">
            <p className="text-slate-400 text-sm py-4">
              {isSearching
                ? `No se encontraron series con "${query}". Probá otro número o estilo.`
                : 'Todavía no hay series repetidas para comparar. Necesitás al menos 2 registros de la misma serie.'}
            </p>
          </Card>
        ) : (
          <>
            ...
            <div className="mb-4">
              <p className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">
                {isSearching ? 'Resultados' : 'Mis series'} ({groups.length})
              </p>
            ...
            {activeGroup && (
              <div className="fade-in">
                ...
                {activeGroup.entries.length >= 2 && (
                  <Card className="mb-4">
                    ...
                    <p className="text-xs text-slate-400 mb-3">
                      Tiempo promedio por repetición · más arriba = mejor
                    </p>
                    <TimeLineChart
                ...
                {/* Análisis */}
                {activeGroup.delta && <DeltaSummaryCard group={activeGroup} />}
                ...
                <div className="flex flex-col gap-3">
                  {[...activeGroup.entries].reverse().map((entry, i) => {
                    ...
                    const delta  = prev ? prev.set.tiempoPromedio - entry.set.tiempoPromedio : null
...
function DeltaSummaryCard({ group }: { group: SimilarSetComparison }) {
  ...
  return (
    <Card className={`border-l-4 ${neutral ? 'border-l-slate-300' : improved ? 'border-l-emerald-400' : 'border-l-amber-400'}`}>
      <div className="flex items-center gap-2 mb-2">
        {neutral  ? <Minus size={18} className="text-slate-400" /> :
         improved ? <TrendingUp size={18} className="text-emerald-600" /> :
